...
(function(global){
  ...
  var DICT = {
    ...
    'btn.paintvec':   ['🧩 塗りをベクター化','🧩 Paint → vector','🧩 涂色转矢量','🧩 칠 → 벡터','🧩 Tô → vector','🧩 Relleno → vector','🧩 Füllung → Vektor','🧩 Remplissage → vecteur','🧩 التعبئة → متجه'],
    'tip.paintvec':   ['塗りつぶした色をベクター図形に変換 → 選択ツールで移動・サイズ調整・削除できます','Convert filled paint into vector shapes you can move, resize and delete with the select tool','将涂色转换为矢量图形，可用选择工具移动、调整大小和删除','칠한 색을 벡터 도형으로 변환 → 선택 도구로 이동·크기조정·삭제','Chuyển vùng tô thành hình vector để di chuyển, đổi cỡ, xóa bằng công cụ chọn','Convierte el relleno en formas vectoriales para mover, redimensionar y borrar con la herramienta de selección','Wandelt Flächen in Vektorformen um, die du mit dem Auswahlwerkzeug verschieben, skalieren und löschen kannst','Convertit le remplissage en formes vectorielles déplaçables, redimensionnables et supprimables avec l’outil de sélection','حوّل التعبئة إلى أشكال متجهة يمكن تحريكها وتغيير حجمها وحذفها بأداة التحديد'],
    'lbl.autovec':    ['自動','Auto','自动','자동','Tự động','Auto','Auto','Auto','تلقائي'],
